# Приложение для управления задачами. Молодцова, Оганян, Нестеров, Марзаганов
## Схема данных Task
...
|---|---|---|
| Task Service | Хранилище задач недоступно | Возврат 500, задача не создана |
| Task Service | Notification Service недоступен | Логирование ошибки, задача создана, возврат 201 |
| Notification Service | Лог-файл недоступен | Запись идёт только в консоль, сервис продолжает работу |
| Notification Service | Невалидное тело запроса | Возврат 422, кастомный обработчик RequestValidationError |
| Notification Service | Ошибка при обработке события | Возврат 500, ошибка логируется, сервис не падает |
